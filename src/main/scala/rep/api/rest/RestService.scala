...
@Api(value = "/system", description = "控制system启停", produces = "application/json")
@Path("system")
class SystemService(ra: ActorRef)(implicit executionContext: ExecutionContext)
  extends Directives {
  ...
  val route = startSystem ~ stopSystem

  @Path("/start/{startCount}")
  @ApiOperation(value = "启动System", notes = "", nickname = "startSystem", httpMethod = "GET")
  @ApiImplicitParams(Array(
    new ApiImplicitParam(name = "startCount", value = "System数量", required = true, dataType = "integer", paramType = "path")))
  @ApiResponses(Array(
    new ApiResponse(code = 200, message = "启动System", response = classOf[QueryResult])))
  def startSystem =
    path("system" / "start" / Segment) { startCount =>
      get {
        complete { (ra ? SystemStart(startCount.toInt)).mapTo[QueryResult] }
      }
    }

  @Path("/stop/{stopFrom}/{stopTo}")
  @ApiOperation(value = "停止System", notes = "", nickname = "stopSystem", httpMethod = "GET")
  @ApiImplicitParams(Array(
    new ApiImplicitParam(name = "stopFrom", value = "起始位置", required = true, dataType = "integer", paramType = "path"),
    new ApiImplicitParam(name = "stopTo", value = "结束位置", required = false, dataType = "integer", paramType = "path")))
  @ApiResponses(Array(
    new ApiResponse(code = 200, message = "停止System", response = classOf[QueryResult])))
  def stopSystem =
    path("system" / "stop" / IntNumber / IntNumber) { (stopFrom, stopTo) =>
      get {
        complete { (ra ? SystemStop(stopFrom, stopTo)).mapTo[QueryResult] }
      }
    }
}
...
class TransactionService(ra: ActorRef)(implicit executionContext: ExecutionContext)
  ...
  val route = getTransaction ~ postSignTransaction
...
class CertService(ra: ActorRef)(implicit executionContext: ExecutionContext)
  ...
  @ApiResponses(Array(
  new ApiResponse(code = 200, message = "查询证书短地址", response = classOf[PostCertAddr])))
  def getAddrByCert =
    path("certAddr" / "getAddrByCert") {
      post {
         entity(as[PostCert]) { PostCert =>
          complete { (ra ? PostCert).mapTo[PostCertAddr] }
        }
      }
    }
  ...
  @ApiResponses(Array(
  new ApiResponse(code = 200, message = "查询证书字符串", response = classOf[PostCertAddr])))
    def getCertByAddr =
    path("certAddr" / "getCertByAddr") {
      post {
         entity(as[PostAddr]) { PostAddr =>
          complete { (ra ? PostAddr).mapTo[PostCertAddr] }
        }
      }
    }
  }
