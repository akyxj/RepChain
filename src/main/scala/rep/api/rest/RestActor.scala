...
object RestActor {
  ...
  case class PostCertAddr(addr: String, cert: String, cid: String, err: String)
  case class QueryResult(result: Option[JValue])
  case class QueryCertAddr(addr: String)
//  case class CSpec(stype: Int, idPath: String, idName: Option[String], iptFunc: String, iptArgs: Seq[String],
//    timeout: Int, secureContext: String, code: String)
...
//    transactionCreator(nodeName, stype, c.idPath, c.iptFunc, c.iptArgs, c.code, c.idName)
  ...
  def hexStringToBytes(hexString: String): Array[Byte] = {
    ...
    val hexs = hexString.toUpperCase()
    val length = hexs.length() / 2
    val hexChars: Array[Char] = hexs.toCharArray()
    val d: Array[Byte] = new Array[Byte](length)
    // 每两个十六进制字符组成一个字节还原到字节数组中
...
//  val preload :ImpDataPreload = ImpDataPreloadMgr.GetImpDataPreload(sTag,"preload")
  ...
  def receive: Receive = {
    ...
    case tranSign(tr: String) =>
      val tr1 = hexStringToBytes(tr) // 解析交易编码后的16进制字符串,进行解码16进制反解码decode
      try {
     ...
     //TODO 做交易的签名验证，是否能在信任列表或kv中找到证书，如果找到验证一下，如果没有找到，则返回错误
      ...
      val tOutSig = txr.withSignature(ByteString.EMPTY)
...
//         sender ! fail
    ...
    case PostCert(pemcert,cid) =>
      ...
      // TODO 根据短地址查询，看看是否有证书在，如果证书在，则返回短地址
      val peercert = ECDSASign.getCertByBitcoinAddr(certaddr)  // 从信任列表中取得证书            
      val certKey = WorldStateKeyPreFix + cid + "_" + PRE_CERT + certaddr  
      ...
      if (peercert != None) {
        ...
        sender ! PostCertAddr(certaddr, pemcertstr, cid, "")
      } else if (kvcer != None){
        ...
        sender ! PostCertAddr(certaddr, "", cid, "不存在证书")
      }
    ...
    case pa: PostAddr =>
      ...
      if(peercert != None) {
        val pemCertPre = new String(Base64.getEncoder.encode(peercert.get.getEncoded))
        val pemcertstr = cert_begin + LINE_SEPARATOR + pemCertPre + LINE_SEPARATOR + end_cert
        sender ! PostCertAddr(pa.addr, pemcertstr, pa.cid, "")
      } else if (kvcer != None){
          if (new String(kvcer.get) == "null") {
            throw new RuntimeException("该用户证书已注销")
          }  
        ...
        sender ! PostCertAddr(pa.addr, "", pa.cid, "不存在证书")
      }
    ...
    case ph: PostHash =>
      ...
      if(res != null) {
        sender ! QueryHash(ph.hash+"已存在，当前值" + res)
      } else {
