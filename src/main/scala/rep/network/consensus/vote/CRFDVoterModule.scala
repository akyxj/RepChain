package rep.network.consensus.vote

import akka.actor.{Actor, Address, Props}
import rep.app.conf.{SystemProfile, TimePolicy}
import rep.crypto.Sha256
import rep.network.base.ModuleBase
import rep.network.consensus.CRFD.CRFD_STEP
import rep.network.consensus.vote.CRFDVoterModule._
import rep.protos.peer.BlockchainInfo
import rep.storage.ImpDataAccess
import rep.utils.GlobalUtils.{ActorType, BlockEvent}
import com.sun.beans.decoder.FalseElementHandler

/**
  *
  * 抽签节点
  * 定时抽签/触发式抽签
  *
  * Created by kami on 2017/6/6.
  */

object CRFDVoterModule {

  def props(name: String): Props = Props(classOf[ CRFDVoterModule ], name)

  //TODO kami 可以加一个index和flag。如果flag为false，则index+1，然后做check。如果为ture则继续投票
  case class NextVote(flag: Boolean,index:Int)

  case object TickBlock

  case object TickVote

  case object VoteRecover

  final case class TickVoteInfo(clusterInfo: Map[ String, BlockchainInfo ])

  case class VoteResult(result: Boolean)

  case class Seed(hash: Array[ Byte ], blkSrc: String)

  case object SeedNode

  case object TransCheckResult {
    val SUC = 1
    val RETRY = 2
    val WAITING = 3
  }

  case object VoteConditionRecheck

}

class CRFDVoterModule(moduleName: String) extends ModuleBase(moduleName) with CRFDVoter{

  import context.dispatcher

  import scala.concurrent.duration._
  import scala.concurrent.forkjoin.ThreadLocalRandom

  val dataaccess: ImpDataAccess = ImpDataAccess.GetDataAccess(pe.getSysTag)

  def rnd = ThreadLocalRandom.current

  //  var seed = Seed(Array[ Byte ](), "")

  var candidatorCur = Set.empty[ Address ]

  //var isBlockVote = false
  
  //var voteblockhash = ""

  var isWaiting = false

  var isSeedNode = false

  var isRecovered = false

  var retryCount = 0

  var trans_num_threshold = 0

  //var blker_index = 1

  //  var isBlockProcessing = false

  var voteCount = 0
  

  
  


  def checkTranNumCondition(transCheckType: Int, curNum: Int, targetNum: Int, retryC: Int): Int = {
    if (curNum < targetNum) {
      if (retryC < SystemProfile.getRetryTime) TransCheckResult.RETRY
      else TransCheckResult.WAITING
    }
    else TransCheckResult.SUC
  }

  def resetTransThreshold(num: Int) = {
    this.trans_num_threshold = num
  }

  override def preStart(): Unit = {
    //    scheduler.scheduleOnce(60.seconds, self, TickBlock)
    this.trans_num_threshold = SystemProfile.getMinBlockTransNum
  }

  // override postRestart so we don't call preStart and schedule a new Tick
  override def postRestart(reason: Throwable): Unit = ()

  override def receive = {
    case NextVote(flag,index) =>
      //println(pe.getSysTag + " vote request")
      retryCount = 0
      //if (!isBlockVote) {
      if (!pe.getIsBlockVote()) {
        if (pe.getStableNodes.size >= SystemProfile.getVoteNoteMin) {
          //isBlockVote = true
          pe.setIsBlockVote(true)
          //voteblockhash = pe.getCurrentBlockHash 
          pe.setVoteBlockHash(pe.getCurrentBlockHash)
          schedulerLink = clearSched()
          if (flag) {
            //blker_index = 1 
            pe.resetBlker_index
          }else {
            //blker_index += 1
            if(index == 0){
              pe.AddBlker_index
            }else{
              pe.setBlker_index(index)
            }
          }
          self ! TickBlock
        }
        else {
          //TODO kami 可以考虑去掉该延迟
          schedulerLink = clearSched()
          schedulerLink = scheduler.scheduleOnce(TimePolicy.getVoteRetryDelay millis,
            self, NextVote(true,0))
        }
      }
      else {
        //if(!pe.getCurrentBlockHash.equals(voteblockhash)){
        if(!pe.getCurrentBlockHash.equals(pe.getVoteBlockHash)){
          //isBlockVote = false
          pe.setIsBlockVote(false)
          //voteblockhash = ""
          pe.setVoteBlockHash("0")
          self ! NextVote(flag,0)
        }
        logMsg(LOG_TYPE.WARN, "Block voting now, waitting for next vote request")
      }

    case TickBlock =>
      logMsg(LOG_TYPE.INFO, "Prepare vote")
      if (pe.getCacheBlkNum() > 0) {
        schedulerLink = clearSched()
        schedulerLink = scheduler.scheduleOnce(TimePolicy.getVoteRetryDelay millis,
          self, TickBlock)
        logMsg(LOG_TYPE.INFO, "RetryDelay vote")
      }
      else {
        checkTranNumCondition(SystemProfile.getTransCreateType, pe.getTransLength(),
          trans_num_threshold, retryCount) match {
          case TransCheckResult.SUC =>
            logMsg(LOG_TYPE.INFO, "Start vote")
            logTime("Vote Start", CRFD_STEP._1_VOTE_START, getActorRef(ActorType.STATISTIC_COLLECTION))
            //各节点执行出块人及候选人抽签
            //val seed = voteblockhash//pe.getCurrentBlockHash
            val seed = pe.getVoteBlockHash//pe.getCurrentBlockHash
            candidatorCur = candidators(pe.getStableNodes, Sha256.hash(seed))
            if (!candidatorCur.isEmpty) {
              pe.resetCandidator(candidatorCur)
              pe.getCacheHeight() match {
                case 1 =>
                  val blo = pe.getSeedNode
                  pe.resetBlocker(blo)
                  isSeedNode match {
                    case true =>
                      getActorRef(ActorType.BLOCK_MODULE) ! (BlockEvent.CREATE_BLOCK, "")
                    case false =>
                      getActorRef(ActorType.BLOCK_MODULE) ! (BlockEvent.BLOCKER, selfAddr)
                  }
                  /*val a = pe.getTmpEndorse
                  if(a != null){
                        logMsg(LOG_TYPE.WARN, "endorse sent from cache in getCacheHeight for 1")
                        getActorRef(ActorType.ENDORSE_MODULE) ! a
                      }else{
                        
                        logMsg(LOG_TYPE.WARN, "endorse sent from cache is null in getCacheHeight for 1")
                      }*/
                      
                  logMsg(LOG_TYPE.INFO, s"Select  in getCacheHeight for 1~ Candidates : ${candidatorCur} ~ Blocker : ${blo}")

                case _ =>
                  //val blo = blocker(candidatorCur, blker_index)
                  val blo = blocker(candidatorCur, pe.getBlker_index)
                  if (blo != None) {
                    pe.resetBlocker(blo.get)
                    //暂时只找到该方法能够明确标识一个在网络中（有网络地址）的节点，后续发现好方法可以完善
                    logMsg(LOG_TYPE.INFO, s"Select  in getCacheHeight for 0~ Candidates : ${candidatorCur} ~ Blocker : ${blo}")
                    if (isThisAddr(selfAddr, blo.get.toString)) {
                      getActorRef(ActorType.BLOCK_MODULE) ! (BlockEvent.CREATE_BLOCK, "")
                      ///run endorse cache
                      /*val a = pe.getTmpEndorse
                      if(a != null){
                        logMsg(LOG_TYPE.WARN, "endorse sent from cache in getCacheHeight for 0")
                        getActorRef(ActorType.ENDORSE_MODULE) ! a
                      }else{
                        logMsg(LOG_TYPE.WARN, "endorse sent from cache is null in getCacheHeight for 0")
                      }*/
                      
                      
                    }
                    else {
                      //如果不是自己则发送出块人地址
                      getActorRef(ActorType.BLOCK_MODULE) ! (BlockEvent.BLOCKER, selfAddr)
                      ///run endorse cache
                      /*val a = pe.getTmpEndorse
                      if(a != null){
                        logMsg(LOG_TYPE.WARN, "endorse sent from cache in getCacheHeight for 0")
                        getActorRef(ActorType.ENDORSE_MODULE) ! a
                      }else{
                        logMsg(LOG_TYPE.WARN, "endorse sent from cache is null in getCacheHeight for 0")
                      }*/
                    }
                  }
                  else {
                    //未选出候选人，是否需要避免该情况
                    logMsg(LOG_TYPE.ERROR, "No Blocker selected")
                    //TODO kami 是否需要重新选举？
                    //blker_index = 1
                    pe.resetBlker_index
                    schedulerLink = clearSched()
                    schedulerLink = scheduler.scheduleOnce(TimePolicy.getVoteRetryDelay millis,
                      self, TickBlock)
                  }
              }
              //isBlockVote = false
              pe.setIsBlockVote(false)
              //voteblockhash = ""
              pe.setVoteBlockHash("0")
              resetTransThreshold(SystemProfile.getMinBlockTransNum)
              logTime("Vote End", CRFD_STEP._2_VOTE_END, getActorRef(ActorType.STATISTIC_COLLECTION))
            }
            else {
              logMsg(LOG_TYPE.INFO, "Not enough candidates")
              schedulerLink = clearSched()
              schedulerLink = scheduler.scheduleOnce(TimePolicy.getVoteRetryDelay millis,
                self, TickBlock)
            }
          case TransCheckResult.RETRY =>
            logMsg(LOG_TYPE.INFO, "Not enough trans")
            println("Not enough trans")
            retryCount += 1
            schedulerLink = clearSched()
            schedulerLink = scheduler.scheduleOnce(TimePolicy.getVoteRetryDelay millis,
              self, TickBlock)

          case TransCheckResult.WAITING =>
            logMsg(LOG_TYPE.INFO, "Not enough trans, waiting for long time")
            isWaiting = true
            //isBlockVote = false
            pe.setIsBlockVote(false)
            //voteblockhash = ""
            pe.setVoteBlockHash("0")
            schedulerLink = clearSched()
            schedulerLink = scheduler.scheduleOnce(TimePolicy.getVoteWaitingDelay millis,
              self, TickBlock)
        }
      }

    case SeedNode =>
      isSeedNode = true
      logMsg(LOG_TYPE.INFO, "This is Seed Node")

    case VoteConditionRecheck =>
      if (isWaiting) {
        checkTranNumCondition(SystemProfile.getTransCreateType, pe.getTransLength(),
          trans_num_threshold, 0) match {
          case TransCheckResult.SUC =>
            isWaiting = false
            schedulerLink = clearSched()
            self ! NextVote(true,0)
          case TransCheckResult.RETRY =>
          //ignore
        }
      }

    //TODO kami 这里不直接改为1，应该进行recheck。而且还应该加个标志位。成功出块复核条件就更爱falg为ture。如果不成功则改为1.然后投票之后再改为配置项2
    case VoteRecover =>
      isWaiting match {
        case true =>
          resetTransThreshold(1)
          self ! VoteConditionRecheck
        case false => //ignore
      }

  }
}
